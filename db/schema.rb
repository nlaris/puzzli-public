# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema[7.1].define(version: 2024_06_20_230830) do
  # These are extensions that must be enabled in order to support this database
  enable_extension "plpgsql"

  create_table "game_tiles", force: :cascade do |t|
    t.bigint "game_id", null: false
    t.bigint "tile_id", null: false
    t.integer "solution_index", default: 0, null: false
    t.index ["game_id", "tile_id"], name: "game_tile", unique: true
    t.index ["game_id"], name: "index_game_tiles_on_game_id"
    t.index ["tile_id"], name: "index_game_tiles_on_tile_id"
  end

  create_table "games", force: :cascade do |t|
    t.string "date"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.integer "num_solutions", default: 0, null: false
    t.index ["date"], name: "index_games_on_date", unique: true
  end

  create_table "tiles", force: :cascade do |t|
    t.string "pattern"
    t.integer "rotation"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["pattern", "rotation"], name: "index_tiles_on_pattern_and_rotation", unique: true
  end

  create_table "user_stats", force: :cascade do |t|
    t.string "user_id"
    t.string "last_completed_date"
    t.integer "streak"
    t.index ["user_id"], name: "index_user_stats_on_user_id", unique: true
  end

  add_foreign_key "game_tiles", "games"
  add_foreign_key "game_tiles", "tiles"
end
